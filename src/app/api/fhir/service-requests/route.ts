...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await FHIRClient.fetchWithAuth(
      `${session.fhirBaseUrl}/ServiceRequest${queryString ? `?${queryString}` : ''}`,
      token
    );
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: 'Failed to fetch service requests', details: errorText },
    ...
    console.error('Error in GET /api/fhir/service-requests:', error);
    return NextResponse.json(
