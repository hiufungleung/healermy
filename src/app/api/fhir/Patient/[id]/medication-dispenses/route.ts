...
export async function GET(
  ...
  try {
    ...
    console.error(`Error in GET /api/fhir/Patient/${patientId}/medication-dispenses:`, error);
    ...
    if (error instanceof Error && (error.message.includes('400') || error.message.includes('501'))) {
      console.warn('MedicationDispense resource not supported by FHIR server');
      return NextResponse.json({ 
        resourceType: 'Bundle',
        type: 'searchset',
        total: 0,
    ...
    console.warn('Returning empty bundle due to error fetching medication dispenses');
    return NextResponse.json(
      {
        resourceType: 'Bundle',
        type: 'searchset',
